...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  actor/.style={draw, ellipse, minimum width=1.6cm, minimum height=1.2cm,
                align=center, font=\sffamily\scriptsize, fill=white},
  uc/.style={draw, ellipse, minimum width=4.0cm, minimum height=0.7cm,
...
% System boundary
\node[sys] at (3.5, 0) {};
\node[font=\sffamily\small\bfseries] at (3.5, 5.7) {StockSense AI System};

% Actors
\node[actor] (owner)  at (-3.0,  0.0) {Store\\Owner};
\node[actor] (engine) at (10.0, -4.0) {n8n Engine};
\node[actor] (ai)     at (10.0,  1.5) {External\\AI Services};

% Use cases inside system
\node[uc] (uc1)  at (3.5,  4.8) {Register / Log In};
\node[uc] (uc2)  at (3.5,  3.8) {Complete Onboarding};
\node[uc] (uc3)  at (3.5,  2.8) {Add / Edit Products};
\node[uc] (uc4)  at (3.5,  1.8) {Smart Scan Product Photo};
\node[uc] (uc5)  at (3.5,  0.8) {Scan Supplier Invoice};
\node[uc] (uc6)  at (3.5, -0.2) {View Dashboard \& KPIs};
\node[uc] (uc7)  at (3.5, -1.2) {View / Dismiss Alerts};
\node[uc] (uc8)  at (3.5, -2.2) {Approve / Dismiss Reorders};
\node[uc] (uc10) at (3.5, -3.2) {Automated Daily Pipeline};
\node[uc] (uc9)  at (3.5, -4.2) {Run Pipeline Manually};
\node[uc] (uc11) at (3.5, -5.2) {Receive WhatsApp Alert};
...
\node[ent] (SL) at (-4.5,0) {System Logs};
\node[ent] (DS) at (4.5,0) {Daily Snapshots};
\node[ent] (PR) at (-4,-3) {Products};
\node[ent] (SU) at (4,-3) {Suppliers};
\node[ent] (SA) at (-7,-6) {Stock Alerts};
\node[ent] (ST) at (-4,-6) {Stock Transactions};
\node[ent] (RS) at (0,-6) {Reorder Suggestions};

% Relationships
\draw[rel] (SP) -- node[pos=0.15, above, lbl]{1} node[pos=0.85, above, lbl]{M} (SL);
\draw[rel] (SP) -- node[pos=0.15, above, lbl]{1} node[pos=0.85, above, lbl]{M} (DS);
\draw[rel] (SP) -- node[pos=0.15, above left, lbl]{1} node[pos=0.85, above left, lbl]{M} (PR);
\draw[rel] (SP) -- node[pos=0.15, above right, lbl]{1} node[pos=0.85, above right, lbl]{M} (SU);

\draw[rel] (PR) -- node[pos=0.15, above left, lbl]{1} node[pos=0.85, above left, lbl]{M} (SA);
\draw[rel] (PR) -- node[pos=0.15, left, lbl]{1} node[pos=0.85, left, lbl]{M} (ST);
\draw[rel] (PR) -- node[pos=0.15, above right, lbl]{1} node[pos=0.85, above right, lbl]{M} (RS);

\draw[rel] (SU) -- node[pos=0.15, above left, lbl]{1} node[pos=0.85, above left, lbl]{M} (RS);
\end{tikzpicture}%
}
...
\label{tab:dbsummary}
{\singlespacing
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{6.5cm} @{}}
\toprule
