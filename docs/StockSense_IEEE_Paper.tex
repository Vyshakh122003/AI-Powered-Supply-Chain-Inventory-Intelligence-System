...
The frontend layer is built with React 19.2.4 and Vite 8.0.0, styled with Tailwind CSS v4, and uses React Router 7.13.1 for navigation. The application provides 12 distinct routes including Dashboard, Products, Alerts, Reorder, Suppliers, and Settings pages.
...
The database layer uses Supabase, a PostgreSQL-based backend-as-a-service platform. The database schema comprises eight tables with proper foreign key relationships and indexing for query performance.
...
The Supabase PostgreSQL database contains eight tables with the following structure:
...
\textbf{WF-03: Inventory Processing \& Stock Depletion} identifies products where current\_stock $\leq$ reorder\_threshold and generates Stock Alerts records. It cleans up existing alerts before creating new ones to avoid duplicates.
