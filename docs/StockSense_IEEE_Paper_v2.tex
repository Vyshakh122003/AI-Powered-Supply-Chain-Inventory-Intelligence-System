...
\begin{abstract}
Managing inventory is a constant struggle for small retail operators in India. Store owners, particularly those running kirana shops, still track stock using notebooks or basic spreadsheets. They rarely have real-time visibility into what is running out, mostly because they cannot afford enterprise-grade software. To tackle these issues, we developed StockSense AI. This system functions as an intelligent inventory manager and supplier decision-support tool. It is built almost entirely on top of free-tier cloud services to minimize costs. Under the hood, a four-layer architecture powers the application: a React frontend, a Supabase PostgreSQL database, n8n for running workflows, and the Groq API invoking Llama 3.1 8B for inference. By orchestrating these components, the pipeline predicts stockout dates, flags items at risk, calculates supplier scores via a Weighted Sum Model, and generates reorder rationale using the LLM. Finally, all this insight gets formatted and pushed automatically to the shop owner as a WhatsApp alert through Twilio. We configured a master workflow to trigger daily at 8:00 AM IST, cascading through eight sub-tasks. We tested the system using a synthesized catalog. For a single store, production deployment stays under Rs.~500 per month. The results indicate that this kind of low-cost orchestration successfully delivers timely intelligence without requiring any manual intervention from the retailer.
\end{abstract}
...
\IEEEPARstart{O}{ver} 12 million kirana (neighborhood) stores form the economic backbone of retail trade across India. Despite their massive footprint, these small and medium enterprises (SMEs) struggle constantly with keeping their shelves stocked properly. Tracking inventory manually offers zero predictive capability. Without insight into demand, shop owners resort to mental math and random physical checks. Consequently, stores end up tying up valuable working capital in overstocked items, or worse, losing customers because a requested product ran completely out of stock.

These operational headaches stem from several structural constraints specific to the Indian market. Local shopkeepers source goods from heavily fragmented supplier networks where delivery speeds fluctuate unpredictably. To make matters worse, affordable tech solutions simply do not exist. Standard enterprise resource planning (ERP) packages are absurdly expensive and far too complicated for a single person to run. While basic cloud point-of-sale applications provide digital ledgers, they lack any meaningful decision-support functionality. They cannot warn an owner when stock will run out, nor can they rank suppliers or offer reorder advice.

We built StockSense AI specifically as an end-to-end intelligence pipeline to bridge this gap for small retailers. The architecture targets five core operational needs. First, tracking inventory levels while mathematically projecting future stockout dates. Second, putting large language models to work to generate contextual reorder advice. Third, figuring out which supplier to call by applying a multi-criteria decision formula. Fourth, routing those critical shortage alerts directly to the owner's WhatsApp inbox. And finally, ensuring the underlying tech stack operates with minimal infrastructure overhead so costs remain negligible.

Our implementation introduces several distinct contributions. We demonstrate a full pipeline---spanning data ingestion through to alert delivery---constructed largely out of free-tier cloud applications. We hook the system directly into Groq's Llama 3.1 8B endpoint to produce logical reasoning for reorder quantities. To handle procurement, we employ a Weighted Sum Model (WSM) that scores suppliers based on price, reliability, and speed metrics. Acknowledging local communication habits, we bypass dashboards and emails entirely, opting instead to deliver notifications via Twilio's WhatsApp API. The entire mechanism is governed by an autonomous daily orchestrator that requires zero input from the owner.

The structure of the report proceeds as follows. Section II examines existing research literature. Section III outlines our socio-economic motivations and the specific technical gaps we address. Section IV explains the proposed architecture alongside the mathematical formulas we rely on. Implementation specifics, such as the n8n workflows and database design, are detailed in Section V. Section VI analyzes the performance results, and Section VII wraps up with thoughts on future upgrades.
...
The proposed system builds upon these research foundations while addressing the specific constraints of small Indian retail stores through a low-cost, WhatsApp-first, AI-enhanced solution architecture.
...
Kirana stores are the cornerstone of the Indian economy, yet they remain largely underserved by modern technology. The primary motivation for StockSense AI is to empower these small business owners who lack the capital for expensive ERP subscriptions. The cost of a single stockout (lost customer) or overstock (expired goods) can significantly impact the thin margins of a neighborhood shop.
...
\begin{figure}[H]
\centering
\textbf{WF-01: Data Ingestion Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
...
\vspace{0.4cm}
\textbf{WF-02: Stockout Date Calculator Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
...
\caption{n8n Execution logic for Data Ingestion (top) and Stockout Prediction (bottom).}
\label{fig:wf01_02}
\end{figure}
...
\begin{figure}[H]
\centering
\textbf{WF-03: Inventory Processing \& Alerts Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=0.4cm and 0.5cm, align=center]
  \node (n1) [wf trigger] {Trigger};
...
\vspace{0.4cm}
\textbf{WF-04: Risk Classification Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=0.4cm and 0.6cm, align=center]
  \node (n1) [wf trigger] {Trigger};
...
\caption{n8n Execution logic for Alerts (top) and Risk Classification (bottom).}
\label{fig:wf03_04}
\end{figure}
...
\begin{figure}[H]
\centering
\textbf{WF-05: AI Reorder Intelligence Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
...
\vspace{0.4cm}
\textbf{WF-06: Supplier Scoring Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=0.4cm and 0.6cm, align=center]
  \node (n1) [wf trigger] {Trigger};
...
\caption{n8n Execution logic for AI Reorders (top) and Supplier Scoring (bottom).}
\label{fig:wf05_06}
\end{figure}
...
\begin{figure}[H]
\centering
\textbf{WF-07: WhatsApp Alert Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=0.4cm and 0.5cm, align=center]
  \node (n1) [wf trigger] {Trigger};
...
\vspace{0.4cm}
\textbf{WF-08: Master Orchestrator Pipeline}\\
\vspace{0.1cm}
\resizebox{\columnwidth}{!}{%
...
\caption{n8n Execution logic for WhatsApp Alerts (top) and Master Orchestration (bottom).}
\label{fig:wf07_08}
\end{figure}
