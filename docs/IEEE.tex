...
\begin{abstract}
Most small retailers in India still use notebooks and spreadsheets to track inventory. They have no way to predict what will sell or when items will run out. And they cannot afford the expensive enterprise software that large stores use.

I solved this by building StockSense AI. It is an inventory manager and supplier decision-support tool. The entire system runs on free and low-cost cloud services.

Here is what it does: predicts stockout dates, flags at-risk items, scores suppliers using a Weighted Sum Model, and explains reorder decisions using an LLM. Alerts go directly to the shop owner's WhatsApp via Twilio.

I ran a daily orchestration that executes eight sequential workflows, complemented by two AI Vision pipelines for onboarding and delivery intake. I tested it with ten products. The monthly cost for a single store is under Rs. 500. The owner gets timely alerts without any extra effort.
\end{abstract}
...
\IEEEPARstart{T}{here} are 12 million kirana stores scattered across India. They form the backbone of neighborhood retail. But most of them struggle with the basics: managing inventory. Manual tracking cannot predict what will sell. Store owners guess. They check stock at random. They run out unexpectedly. When a product runs out, they lose customers. When they overstock, items expire and money is wasted.
...
The \textbf{Frontend Layer} is a React 19 Single Page Application (SPA), built with Vite 8 and Tailwind CSS v4. The shop owner gets various interfaces : an inventory dashboard showing stock levels and health scores, an AI reorder module for reviewing suggestions, and a supplier analytics page. The frontend talks to the workflow engine via REST webhooks. It connects to Supabase for authentication and real-time updates.

The \textbf{Workflow Engine Layer} runs in n8n, an open-source automation platform. Ten workflows handle the entire business logic. Here's what each one does:
...
\item \textit{WF-08} (Master Orchestrator): Runs daily at 8:00 AM IST, executes WF-02 through WF-07 in order.
\item \textit{WF-09} (AI Vision Onboarding): Extracts product data from photos for automated inventory setup.
\item \textit{WF-10} (Smart Scan Invoices): Processes supplier bills to automatically update stock levels.
\end{itemize}

The \textbf{Database Layer} uses Supabase (managed PostgreSQL) with real-time subscriptions via WebSocket. The schema has eight tables: Store Profiles, Products, Suppliers, Stock Alerts, Reorder Suggestions, Stock Transactions, Daily Snapshots, and System Logs. The operational data tables reference a store\_id field for future multi-store support, while Store Profiles acts as the parent store table. When inventory or alerts change, real-time subscriptions push updates to the dashboard instantly. No stale data.

The \textbf{AI/External Services Layer} integrates three external APIs. Groq's Llama 3.1 8B model generates reorder reasoning, Google Gemini 2.5 Flash supports image-based product and invoice extraction, and Twilio's WhatsApp Business API sends alerts. In WF-05, I pass product data, stock levels, and supplier info to Groq. It returns a natural-language explanation of why to reorder. In WF-09 and WF-10, Gemini extracts structured data from uploaded images. In WF-07, alerts are formatted and sent via WhatsApp. The owner gets critical notifications instantly on their phone.

All layers talk to each other via HTTP REST and standard database drivers. This loose coupling lets me test each layer independently. If I need to swap out Supabase for PostgreSQL, I can do it without touching the workflows. Similarly, n8n can be replaced with another workflow platform later.
...
The proposed system is developed using a modern, low-cost technology stack carefully selected for accessibility and performance. The frontend application is built using \textbf{React 19} and \textbf{Vite 8}, with responsive styling implemented via \textbf{Tailwind CSS v4}. The backend architecture combines serverless database services with a self-hosted workflow engine, relying on \textbf{Supabase (PostgreSQL)} for robust, real-time database management and authentication. Complex business logic and orchestration are handled entirely by the \textbf{n8n} workflow automation engine. For AI-driven decision-making, the system integrates the \textbf{Groq API} hosting the Llama~3.1 8B model, ensuring high-speed inference for reorder suggestions. It also uses \textbf{Google Gemini 2.5 Flash} for product and invoice image extraction. Finally, the \textbf{Twilio API} is utilized for automated WhatsApp alert delivery.

\subsection{Dataset}
I tested the system with a synthesized dataset that mimics a real kirana store. The dataset includes a catalog of ten fast-moving consumer goods (FMCG) items with daily sales velocities, reorder thresholds, and current stock levels. I also created four supplier profiles with reliability and delivery speed metrics to test the supplier scoring model. This setup lets me validate edge cases systematically: zero sales days, stockouts, and delivery delays.
...
The implemented system has been tested with sample data including ten products and four suppliers. Key performance characteristics are summarized in Table~\ref{tab:perf}.
...
store with eight out-of-stock products scores 18,
